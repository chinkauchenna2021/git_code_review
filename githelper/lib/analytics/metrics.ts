...
export class EventTracker {
  ...
  private static async triggerCriticalAlert(data: any): Promise<void> {
    try {
      ...
      logger.info('Critical security alert triggered', alertData)
    } catch (error) {
  ...
  private static async triggerErrorAlert(data: ErrorEventData): Promise<void> {
    try {
      ...
      logger.info('Critical error alert triggered', alertData)
    } catch (error) {
  ...
  static async getEventsByType(eventType: string, startDate: Date, endDate?: Date): Promise<any[]> {
    try {
      ...
      return events.map((event: { newValues: any }) => event.newValues)
    } catch (error) {
  ...
  static async exportEvents(
    ...
    try {
      ...
      return events.map((event: { id: any; createdAt: any; newValues: any; action: any; entity: any; userId: any; user: { name: any; email: any }; metadata: any }) => ({
        id: event.id,
